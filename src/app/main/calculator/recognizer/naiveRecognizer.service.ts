...
export class NaiveRecognizerService {
	...
	recognizeNaive(tiles: Tile[], setsX: Set[]): Hand[] {
		...
		allPossibleSets.forEach((currentPair: Set,suitSets: Tile[][]) => {
			var sets: Set[] = [ new Set(currentPair.tile, SetType.Jantou, false, "", -1, [new Tile(currentPair.tile.value, currentPair.tile.suit, currentPair.tile.name), new Tile(currentPair.tile.value, currentPair.tile.suit, currentPair.tile.name)]) ];
	...
	predict(predictionGrouping: PredictionGrouping): void {
		...
		while(!this.isEmpty(predictionGrouping.expectedSuitSets)) {
			...
			if(!topMatchesBySuit[suitToRemove]) {
				break;
			}
		...
		predictionGrouping.tiles = [];
		
		for(let set of addedSets.splice(0)) {
		...
		this.updateDora(predictionGrouping);
		
		var hand = new Hand(predictionGrouping.tiles, predictionGrouping.sets);
		hand.isPredicted = true;
	...
	updateDora(predictionGrouping: PredictionGrouping) {
		for(let tile of this.extraInfoService.realDoraTiles) {
			for(let set of predictionGrouping.sets) {
				for(let setTile of set.getTiles()) {
					if(tile.equals(setTile)) {
						setTile.isDora = tile.isDora;
					}
				}
			}
		}
	...
	generateAllPossibleSetsForTile(tile: Tile, generatePair: boolean): Set[] {
		...
		if(generatePair) {
			possibleSets.push(new Set(tile, SetType.Jantou, false, "", -1, [tile, new Tile(tile.value, tile.suit, tile.name)]));
		}
	...
	generatePon(tile: Tile): Set {
		var tilesInPon: Tile[] = [];
		tilesInPon.push(new Tile(tile.value, tile.suit, tile.name));
		tilesInPon.push(new Tile(tile.value, tile.suit, tile.name));
		tilesInPon.push(new Tile(tile.value, tile.suit, tile.name));
