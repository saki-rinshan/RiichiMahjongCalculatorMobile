	...
	removeDora(tileIndex: number): void {
		...
		if (index > -1) {
			if(this.doraTiles.filter(tilex => tilex.suit == tile.suit && tilex.value == tile.value).length == 1){
				this.editDoraValue(tile, false);
			}
			this.doraTiles.splice(index, 1);
		}
	}
	
	editDoraValue(doraMarker: Tile, makeDora: boolean): void {
		var dora: Tile = this.getDoraFromIndicator(doraMarker);
		for(let row of this.tileList.rows) {
			var actualDora: Tile[] = row.filter(tile => tile.equals(dora));
			var found: boolean = false;
			for(let tile of actualDora) {
				this.realDoraTiles.push(tile);
				tile.isDora = makeDora;
				found = true;
			}
			if(found) {
				return;
