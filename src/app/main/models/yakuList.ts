...
export class YakuList {
	...
	specYaku: Map<Hand, Yaku> = new Map<Hand, Yaku>();
	specYakuhai: Map<Hand, Yaku> = new Map<Hand, Yaku>();
	...
	validate(hand: Hand): Yaku[] {
		...
		var yakuhai: Yakuhai = new Yakuhai(this.extraInfoService, this.extraInfoService.languageService.currentLanguage);
		this.specYakuhai.set(hand, yakuhai);
		...
		this.specYakuhai.forEach((dora: Yaku, specHand: Hand) => {
			if(hand === specHand) {
				if(dora.validate(hand)) {
					matchedYaku.push(dora);
					hand.addYaku(dora);
				}
			}
		});
	...
	validateOther(hand: Hand): Yaku[] {
		var matchedYaku: Yaku[] = new Array<Yaku>();
		var dora = new Dora(this.extraInfoService, this.extraInfoService.languageService.currentLanguage);
		this.specYaku.set(hand, dora);
		...
		this.specYaku.forEach((dora: Yaku, specHand: Hand) => {
			if(hand === specHand) {
				if(dora.validate(hand)) {
					matchedYaku.push(dora);
					hand.addYaku(dora);
